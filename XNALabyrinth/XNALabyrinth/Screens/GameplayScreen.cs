...
namespace XNALabyrinth
{
    ...
    class GameplayScreen : GameScreen
    {
        ...
        {
            vibrationTime = time = TimeSpan.Zero;
                                                       ...
                                                       bool coveredByOtherScreen)
        {
            ...
            if (!isPaused)
            {
                ...
                vibrationTime += gameTime.ElapsedGameTime;
            }

            if (vibrationTime > TimeSpan.FromSeconds(1))
            {
                vibrationTime = TimeSpan.Zero;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            /// Print info during game play
            if (!game.isCube && !isPaused)
                PrintOnScreen(gameTime);
            
        }

        protected void PrintOnScreen(GameTime gameTime)
        {

            spriteBatch.Begin();

            // Draw Hello World
            string output = time.Minutes.ToString("D2") + ":" + time.Seconds.ToString("D2");

            //Vector2 FontPos = new Vector2(game.GraphicsDevice.Viewport.Width / 2,
              //                              game.GraphicsDevice.Viewport.Height / 2);

            // Find the center of the string
            //Vector2 FontOrigin = ScreenManager.Font.MeasureString(output) / 2;
            
            // Draw the string
            spriteBatch.DrawString(ScreenManager.Font, output, Vector2.Zero, Color.LightGreen,
